...
// 399 => Shortest Distance to Target String in a Circular Array
...
const ClosesTarget = (words, target, startIndex) => {
...
// 407 => Maximize Sum of At Most K Distinct Elements
...
const maxKDistinct = (nums, k) => {
  const uniques = [...new Set(nums)].sort((a, b) => a - b);
  const kelement = uniques.slice(-k).sort((a, b) => b - a);
  return kelement;
};
...
// 408 => Earliest Time to Finish One Task
...
const earliestTime = (nums) => {
  const times = [];

  for (let [x1, x2] of nums) {
    const time = x1 + x2;
    times.push(time);
  }
...
// 409 => Smallest Absent Positive Greater Than Average
...
const smallestAbsent = (nums) => {
  const average = nums.reduce((sum, current) => current + sum, 0) / nums.length;

  let x = 0;

  while (nums.includes(x) || x <= average) {
    x++;
  }
  return x;
};
...
// 420 => Keep Multiplying Found Values by Two
...
// 426 => You are given a license key represented as a string s that consists of only alphanumeric characters and dashes. The string is separated into n + 1 groups by n dashes. You are also given an integer k.
...
const licenseKeyFormatting = (s, x) => {
  ...
  for (let i = removeDashes.length - 1; i >= 0; i--) {
    result += removeDashes[i];
    count++;
...
const sumOffGoodNumbers = (arr, k) => {
  let goodNumSum = 0;

  function isGoodIndex(index, current) {
    if (index < 0 || index >= arr.length) return true;
    return current > arr[index];
  }

  for (let i = 0; i < arr.length; i++) {
    if (isGoodIndex(i - k, arr[i]) && isGoodIndex(i + k, arr[i])) {
      goodNumSum += arr[i];
    }
  }
...
// 439 => Smallest Index With Digit Sum Equal to Index
...
const smallestIndex = (nums) => {
  ...
  return Math.min(...indeces) === Infinity ? -1 : Math.min(...indeces);
};
...
// 440 => Greatest English Letter in Upper and Lower Case
...
const greatestLetter = (s) => {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (s.includes(upperCaseLetter) && s.includes(lowerCaseLetter)) {
      appearLetter.push(upperCaseLetter);
    }
  }

  if (appearLetter.length === 0) return '';

  appearLetter.sort();
  return appearLetter[appearLetter.length - 1];
};
...
const traingleType = (arr) => {
  ...
  // Step 3: Decide type
  for (let key in obj) {
    if (obj[key] === 3) return 'equilateral';
    if (obj[key] === 2) return 'isosceles';
  }
...
// 442 => Smallest Missing Multiple of K
...
// console.log(missingMultiple([1, 4, 7, 10, 15],, )1
